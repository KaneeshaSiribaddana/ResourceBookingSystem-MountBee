...
function userSignIn() {
    ...
    if (userName.value.length == 0) {
        validateDiv.innerHTML = "Please Enter Your Username";
    } else if (userPassword.value.length == 0) {
        validateDiv.innerHTML = "Please Enter Your Password";
    } else {
        var formData = new FormData();
        ...
        r.onreadystatechange = function() {
            if (r.readyState == 4) {
                var t = r.responseText;
                if (t == "success") {
                    window.location = "http://localhost:8012/resource_booking_system/ADMIN/PHP/home.php";
                } else {
                    validateDiv.innerHTML = t;
                }
            }
        };
        r.open("POST", "http://localhost:8012/resource_booking_system/ADMIN/PHPHANDLER/db.signinprocess.php", true);
        r.send(formData);
